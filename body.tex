...
T &= \frac{1}{2}\rho A\int_{0}^{L}\left\langle(\dot u-\Omega v)\textbf{i}+[\dot v+\Omega(a+x+u)]\textbf{j}+ \dot w\textbf{k}\right\rangle^2\text{ d}x, \\
  &= \frac{1}{2}\rho A\int_{0}^{L} (\dot u-\Omega v)^2+[\dot v+\Omega\cdot(a+x+u)]^2+\dot w^2\text{ d}x.
  \label{eq:kin.final}
\end{align}
...
We now move to generalized coordinate frame $q_i$, where subscript $i$ represents the different coordinate directions. In the kinetic energy expression given by Eqn.~\ref{eq:kin.final}, there are five dynamic expressions to consider: $\dot u, \dot v, \dot w, u, v$. In the generalized coordinates, this corresponds to $\dot q_i$, for $i=\{1,2,3\}$, and $q_i$, for $i=\{1,2\}$. When the chain rule is applied to the variation operator, we find
\begin{equation}
\delta \hat T(\dot q_i,q_i) = \frac{\partial \hat T}{\partial \dot q_i}\delta \dot q_i+\frac{\partial \hat T}{\partial q_i}\delta q_i.
\label{eq:kinetic.variation.initial}
\end{equation}
...
\end{equation}
In Eqn.~\ref{eq:kinetic.variation.initial}, the variation on the $\dot q_i$ components yields the variation $\delta \dot q_i$. However, in order for variations to be applied so that the test function vanishes, all variation test functions must be in the same form. To accomplish this, the $\dot q_i$ term is integrated by using integration by parts. This is accomplished by utilizing the time integral; the spatial integral shall be excluded for brevity.
...
\end{equation}
The variations $\delta h_v$ and $\delta h_w$ are not in the proper frame; that is, we need variations on $s,v,w$ only. Since $h_v,h_w$ are functions of $v,w$ respectively, we can calculate the variations as follows.
\begin{equation}
...
\end{equation}
In our analysis we may now substitute this result for $\delta u$ and continue the process for strain energy.
...
\begin{eqnarray}
\left.-\frac{\text d}{\text dt}\left[\frac{\partial T}{\partial \dot q_i}\right]\right|_{q_i=s} 
&=& -\left(\frac{\rho A}{2}\right)\frac{\text d}{\text dt}\int_0^L[2\dot s-2(\dot h+\Omega v)]\text{ d}x,\\
&=& -\rho A\int_0^L [\ddot s-\dot \Omega v-\Omega \dot v-\ddot h]\text{ d}x,
\end{eqnarray}
where $\ddot h$ is defined as
\[\ddot h = \ddot h_v+\ddot h_w.\]
Continuing, we have
\begin{eqnarray}
\left.-\frac{\text d}{\text dt}\left[\frac{\partial T}{\partial \dot q_i}\right]\right|_{q_i=v}
&=& -\left(\frac{\rho A}{2}\right)\frac{\text d}{\text dt}\int_0^L[2\dot v+2\Omega (a+x+s-h)]\text{ d}x,\\
&=& -\int_0^L \rho A [\ddot v + \dot \Omega (a+x+s-h)+\Omega(\dot s-\dot h)]\text{ d}x.
\end{eqnarray}
In $w$,
\begin{eqnarray}
...
\end{eqnarray}
Next we shall analyze the second term. In $s$,
\begin{eqnarray}
\left.\frac{\partial T}{\partial q_i}\right|_{q_i=s} 
&=& \frac{\rho A}{2}\int_0^L 2\dot v\Omega+2\Omega^2(s+a+x)-2\Omega^2h\text{ d}x,\\
&=& \rho A\int_0^L  [\dot v\Omega+\Omega^2(a+x+s-h)]\text{ d}x.
\end{eqnarray}
In $v$,
\begin{eqnarray}
\left.\frac{\partial T}{\partial q_i}\right|_{q_i=v} 
&=& \frac{\rho A}{2}\int_0^L [-2\Omega\dot s-2\Omega\dot h+2\Omega^2v ]\text{ d}x,\\
&=& \rho A\int_0^L  [\Omega^2v-\Omega(\dot u+\dot h)] \text{ d}x.
\end{eqnarray}
Finally, in $w$,
\begin{eqnarray}
...
Firstly, we write down the $s$ coordinate equations.
\begin{eqnarray}
& & -\int_0^L \rho A \left[\ddot s-\frac{\partial}{\partial t}(\dot h_v+\dot h_w)-\dot \Omega v-\Omega \dot v\right]\text{ d}x +\nonumber \\
& & \qquad \int_0^L \rho A [\lbrace s-(h_v+h_w)+a+x\rbrace\Omega^2+\dot v\Omega]\text{ d}x +\\
& & \qquad \qquad \int_0^L EA\left[ s_{xx} \right]+EA_x\left[ s_{x} \right]\text{ d}x = 0.\nonumber
\end{eqnarray}
Canceling integrals and simplifying yields the following.
\begin{equation}
\rho A \left[\ddot s-\frac{\partial}{\partial t}(\dot h_v+\dot h_w)-\dot \Omega v-\lbrace s-(h_v+h_w)+a+x\rbrace\Omega^2\right] = EA[s_{xx}]+EA_x[s_{x}].
\end{equation}

Next, we write the $v$ coordinate equations.
\begin{eqnarray}
& & -\int_0^L \rho A [\ddot v + \dot \Omega (a+x+s-(h_v+h_w))+\Omega(\dot s - (\dot h_v+\dot h_w))]\text{ d}x +\nonumber \\ 
& & \qquad \int_0^L \rho A [-\Omega(\dot s-(\dot h_v+\dot h_w))+\Omega^2v] \text{ d}x -\\
& & \qquad E\int_0^L [I_{zz}]_{xx}[v_{xx}]+2[I_{zz}]_{x}[v_{xxx}]+[I_{zz}][v_{xxxx}]\text{ d}x = 0.\nonumber
\end{eqnarray}

Lastly, we write the $w$ coordinate equations.
\begin{eqnarray}
& & -\int_0^L \rho A [\ddot w]\text{ d}x + \\
& & \qquad -E\int_0^L [I_{yy}]_{xx}[w_{xx}]+2[I_{yy}]_x[w_{xxx}] + [I_{yy}][w_{xxxx}]\text{ d}x = 0. \nonumber
\end{eqnarray}
...
\textbf{{DISCUSSION: SHOW THAT S,V,W SATISFY THE ABOVE BC's}}
